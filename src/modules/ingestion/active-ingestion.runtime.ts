...
export class ActiveIngestionRuntime {
  ...
  async start(): Promise<void> {
    ...
    try {
       ...
       * Kafka admission 실패 시 upstream tick은 realtime Redis에도
       * 전달되지 않으므로 producer connection 실패 시 upstream을 열지 않는다.
       */
       ...
       *
       * WebSocket, reconnect timer, heartbeat, active message handlers는
       * 이전 Epoch와 공유되지 않는다.
       */
  ...
  async stop(): Promise<void> {
     ...
     *
     * 이 작업이 끝난 뒤에는 새로운 raw tick Kafka admission이
     * 발생하지 않아야 한다.
     */
    if (twelveDataStream) {
      try {
    ...
    /*
     * 2. 모든 active message handler가 settle된 뒤 Kafka producer를
    ...
    /*
     * 3. 마지막으로 realtime publisher를 종료한다.
     */
    if (publisher) {
      try {
